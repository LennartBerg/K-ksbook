...
class UserListPDOSQLite implements UserListDAO
{
    ...
    public function newUser($username, $password, $email, $description)
    {
        try {
            ...
            $command = $db->prepare($sql);
            $command->execute([":username" => $username]);
            if ($command->fetchColumn() > 0) {
    ...
    public function getUserByID($userID)
    {
        try {
            ...
            return new User($result["userID"], $result["username"], $result["password"], $result["email"], $result["description"]);
        } catch (PDOException $exc) {
            throw new InternalUserDataBaseError();
        }
    }
    ...
    public function getAllUsers()
    {
        try {
            ...
            foreach ($result as $row) {
                $users[] = new User($row["userID"], $row["username"], $row["password"], $row["email"], $row["description"]);
            }
            return $users;
        } catch (PDOException $exc) {
            throw new InternalUserDataBaseError();
